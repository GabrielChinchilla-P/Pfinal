  ...
  <div class="card-body">
    ...
    <form action="<?= base_url('nomina/update/' . $nomina->id_nomina); ?>" method="post" class="row g-3">
        ...
        <select name="id_empleado" id="id_empleado" class="form-select" required>
          ...
          <?php foreach ($empleados as $empleado): ?>
            <option value="<?= $empleado->id_empleado; ?>" 
              <?= ($empleado->id_empleado == $nomina->id_empleado) ? 'selected' : ''; ?>>
      ...
      <div class="col-md-3">
        ...
        <input type="number" step="0.01" class="form-control" id="salario_base" 
               name="salario_base" value="<?= esc($nomina->salario_base); ?>" required>
      </div>

      <div class="col-md-3">
        <label for="bonificacion" class="form-label">Bonificación (Q)</label>
        <input type="number" step="0.01" class="form-control" id="bonificacion" 
               name="bonificacion" value="<?= esc($nomina->bonificacion); ?>" required>
      </div>

      <div class="col-md-3">
        <label for="igss" class="form-label">IGSS (Q)</label>
        <input type="number" step="0.01" class="form-control" id="igss" 
               name="igss" value="<?= esc($nomina->igss); ?>" readonly>
      </div>

      <div class="col-md-3">
        <label for="sueldo_liquido" class="form-label">Sueldo Líquido (Q)</label>
        <input type="number" step="0.01" class="form-control" id="sueldo_liquido" 
               name="sueldo_liquido" value="<?= esc($nomina->sueldo_liquido); ?>" readonly>
      </div>

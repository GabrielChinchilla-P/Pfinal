...
<div class="card shadow-sm mb-4 border-0">
    <div class="card-header bg-white border-bottom d-flex justify-content-between align-items-center">
        <h3 class="h5 mb-0 text-primary"><i class="fa-solid fa-file-invoice-dollar me-2"></i> Listado de Informes de Gastos</h3>
    ...
    <div class="card-body">
        ...
        <form action="<?= site_url('informegasto/buscar') ?>" method="get" class="row g-2 mb-4">
            <div class="col-md-4">
                <input type="text" name="q" class="form-control" placeholder="Empleado o Departamento" value="<?= esc($searchQuery ?? '') ?>">
            </div>
            <div class="col-md-3">
            ...
            <div class="col-md-2 d-flex">
                <button type="submit" class="btn btn-outline-secondary me-2"><i class="fa-solid fa-magnifying-glass"></i> Buscar</button>
                <?php if (isset($searchQuery) || isset($fecha_inicio) || isset($fecha_fin)): ?>
                    <a href="<?= site_url('informegasto') ?>" class="btn btn-outline-danger" title="Limpiar búsqueda"><i class="fa-solid fa-xmark"></i></a>
                <?php endif; ?>
            </div>
        </form>
            ...
            <table class="table table-striped table-hover align-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="<?= $total > 1000 ? 'text-danger fw-bold' : '' ?>">Q<?= number_format($total,2) ?></td>
                        <td><?= esc(substr($i['descripcion'] ?? '', 0, 50)) ?><?= strlen($i['descripcion'] ?? '') > 50 ? '...' : '' ?></td>
                        <td>
                            <a href="<?= site_url('informegasto/edit/' . ($i['id_informe'] ?? '')) ?>" class="btn btn-primary btn-sm me-1">
                            ...
                            <form id="delete-form-<?= esc($i['id_informe'] ?? 'temp') ?>" action="<?= site_url('informegasto/delete/' . ($i['id_informe'] ?? '')) ?>" method="post" style="display:none;">
